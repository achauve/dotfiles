...
subscribe modifiers.LWin, key.T do # change layout to Tile
	windawesome.current_workspace.change_layout Windawesome::TileLayout.new
end

subscribe modifiers.LWin, key.M do # change layout to Full Screen
	windawesome.current_workspace.change_layout Windawesome::FullScreenLayout.new
end

subscribe modifiers.LWin, key.F do # change layout to Floating
...
(1 .. config.workspaces.length).each do |i|
	...
	subscribe modifiers.LWin | modifiers.Alt | modifiers.Shift, k do
		windawesome.add_application_to_workspace Windawesome::NativeMethods.get_foreground_window, i
	end
end

subscribe modifiers.LWin, key.Left do
    begin
        cur_id = windawesome.current_workspace.id
        next_id = [cur_id - 1, 1].max
        windawesome.switch_to_workspace next_id
    rescue Exception=>e
    end
end

subscribe modifiers.LWin, key.Right do
    begin
         cur_id = windawesome.current_workspace.id
         next_id = [cur_id + 1, config.workspaces.length].min
         windawesome.switch_to_workspace next_id
    rescue Exception=>e
    end
end
